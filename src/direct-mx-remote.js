...
export class RemoteDirectMxTransport {
  #deliveryToken;
  constructor({ workerUrl, deliveryToken, fetcher = globalThis.fetch, signingResolver, timeoutMs = 45_000 } = {}) {
    this.kind = "direct-mx"; this.workerUrl = String(workerUrl || "").replace(/\/$/, ""); this.#deliveryToken = deliveryToken;
    this.fetcher = fetcher; this.signingResolver = signingResolver; this.timeoutMs = timeoutMs;
  }
  async send(message) {
    const signing = await this.signingResolver?.(message);
    const job = { version: 1, messageId: message.message_id, attempt: message.attempt || 1,
      ...
      correlation: { tenantId: message.tenant_id || null, idempotencyKey: message.idempotencyKey || null },
      signing: signing ? { domain: signing.domain, selector: signing.selector } : null };
    let response;
    try { response = await this.fetcher(`${this.workerUrl}/v1/deliver`, { method: "POST", signal: AbortSignal.timeout(this.timeoutMs),
    ...
    if (!response.ok) {
      ...
      return { status: retryable ? "retry" : "failed", transport: "direct-mx",
        error: { code: result?.error || "MAIL_DIRECT_MX_WORKER_REJECTED", message: result?.message || "Direct-MX worker rejected delivery" } };
    }
