...
export interface MailMessage { message_id:string; application_id:string; tenant_id?:string|null; identity:string; from:string; to:string[]; cc:string[]; bcc:string[]; reply_to:string[]; subject:string; text:string; html:string; template?:string|null; variables:Record<string,unknown>; metadata:Record<string,unknown>; status:MailStatus; provider?:string|null;providerMessageId?:string|null;created_at:string; updated_at:string; links:string[]; events?:MailEvent[] }
export interface DeliveryResult { status:"accepted"|"sent"|"retry"|"failed"; error?:unknown; message_id?:string;provider?:string;providerMessageId?:string|null;metadata?:Record<string,unknown> }
