...
export interface RemoteDirectMxOptions { workerUrl:string;deliveryToken:string;timeoutMs?:number;fetcher?:typeof fetch;signingResolver?:(message:MailMessage)=>Promise<Record<string,unknown>|null> }
export class RemoteDirectMxTransport implements MailTransport { readonly kind:"direct-mx";constructor(options:RemoteDirectMxOptions);send(message:MailMessage):Promise<DeliveryResult> }
export function createRemoteDirectMxTransport(options:RemoteDirectMxOptions):RemoteDirectMxTransport;
